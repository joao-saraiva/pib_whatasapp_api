...
RSpec.describe PlayerPerMatch, type: :model do
  ...
  it {
    should define_enum_for(:status).with_values(%i[confirmed waiting gived_up])
  }
  ...
  describe '#give_up!' do
    ...
    context 'with other playes that are below the current position' do
      it 'should decrease - 1 position to everyone below the current position' do
        ...
        10.times do
          last_player_per_match = FactoryBot.create(:player_per_match,
                                                    match_id: @match.id)
        end
        ...
        expect(@match.player_per_matches.avaliable.pluck(:position).sort).to eq([
